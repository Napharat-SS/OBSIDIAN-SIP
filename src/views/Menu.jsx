...
export default function MenuPage() {
  ...
  const filteredItems = useMemo(() => {
    return activeCat === "all"
      ? MenuItem
      : MenuItem.filter((p) => (p.category || "").toLowerCase() === activeCat);
  }, [activeCat]);
  ...
  return (
    <div className="min-h-screen bg-[#0f0f10] text-white">
      <div className="container mx-auto px-4 py-10">
        ...
        <header className="py-15 px-20 mb-10 rounded-4xl bg-[url('/bgMenu.png')] bg-cover bg-bottom bg-no-repeat">
          ...
          <p className="mt-4 max-w-2xl mx-auto text-center text-bold font-kanit text-sm md:text-xl drop-shadow">
    ...
    rounded-xl md:px-6 md:py-7 transition-colors duration-300 px-3 py-5
    text-gray-100
    ...
    data-[state=active]:font-semibold"
              ...
              {CATEGORY_ORDER.map((cat) => (
                <TabsTrigger
                  key={cat.id}
                  value={cat.id}
                  className="flex flex-col items-center justify-center rounded-xl text-xs md:text-sm
                     data-[state=active]:bg-white data-[state=active]:text-black
                     data-[state=active]:font-semibold text-gray-100 md:px-6 md:py-7 py-5 px-3 transition-colors duration-300"
                >
              ...
              {filteredItems.map((product) => (
                <MenuCard
                  ...
                  onFavoriteToggle={(isFavorite) =>
                    console.log(
                      `Product ${product.id} favorite status: ${isFavorite}`
                    )
                  }
                />
              ))}

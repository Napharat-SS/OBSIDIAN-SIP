...
const Navbar = () => {
  ...
  return (
    <nav
      className="flex justify-between items-center py-2 px-4 lg:px-20 
    bg-gradient-to-r from-[#472C03] to-[#A36403] shadow-[0_4px_6px_rgba(0,0,0,0.6)]"
    >
      <Link
        to="/"
      ...
      {/* mobile menu nav*/}
      {isMenuOpen && (
        <div className="absolute top-12 left-0 w-full bg-gradient-to-r from-[#472C03] to-[#A36403] flex flex-col items-center gap-4 py-6 md:hidden z-50 ">
          {mobileNav.map((item) => (
            <Link
              key={item.path}
              to={item.path}
